...
public class LightingHooks {
    ...
    public static void relightSkylightColumn(final World world, final WorldChunk chunk, final int x, final int z, final int height1, final int height2) {
        ...
        if (sections[yMin >> 4] == WorldChunk.EMPTY && yMin > 0) {
            world.getBrightness(LightType.SKY, new BlockPos(xBase, yMin - 1, zBase));
        }
        ...
        if (emptySections != 0) {
            for (final Direction dir : Direction.Plane.HORIZONTAL) {
                ...
                final boolean neighborColumnExists = (((x + xOffset) | (z + zOffset)) & 16) == 0
                        ...
                        || world.getChunkSource().getLoadedChunk(chunk.chunkX + xOffset, chunk.chunkZ + zOffset) != null;
    ...
    private static void scheduleRelightChecksForColumn(final World world, final LightType lightType, final int x, final int z, final int yMin, final int yMax) {
        ...
        for (int y = yMin; y <= yMax; ++y) {
            world.getBrightness(lightType, pos.set(x, y, z));
        }
    }
    ...
    public static void scheduleRelightChecksForChunkBoundaries(final World world, final WorldChunk chunk) {
        for (final Direction dir : Direction.Plane.HORIZONTAL) {
            ...
            final WorldChunk nChunk = world.getChunkSource().getLoadedChunk(chunk.chunkX + xOffset, chunk.chunkZ + zOffset);
    ...
    private static void scheduleRelightChecksForBoundary(final World world, final WorldChunk chunk, WorldChunk nChunk, WorldChunk sChunk, final LightType lightType, final int xOffset, final int zOffset, final Direction.AxisDirection axisDir) {
        ...
        if (nChunk == null) {
            nChunk = world.getChunkSource().getLoadedChunk(chunk.chunkX + xOffset, chunk.chunkZ + zOffset);
        ...
        if (sChunk == null) {
            sChunk = world.getChunkSource().getLoadedChunk(chunk.chunkX + (zOffset != 0 ? axisDir.getOffset() : 0), chunk.chunkZ + (xOffset != 0 ? axisDir.getOffset() : 0));
    ...
    public static void readNeighborLightChecksFromNBT(final WorldChunk chunk, final NbtCompound nbt) {
        if (nbt.contains(neighborLightChecksKey, 9)) {
            ...
            if (list.size() == FLAG_COUNT) {
                ...
                PhosphorMod.LOGGER.warn("WorldChunk field {} had invalid length, ignoring it (chunk coordinates: {} {})", neighborLightChecksKey, chunk.chunkX, chunk.chunkZ);
            }
        }
    }
    ...
    public static void initChunkLighting(final WorldChunk chunk, final World world) {
        ...
        final BlockPos.PooledMutable pos = BlockPos.PooledMutable.of(xBase, 0, zBase);
        ...
        if (world.isAreaLoaded(pos.add(-16, 0, -16), pos.add(31, 255, 31), false)) {
            ...
            for (int j = 0; j < sections.length; ++j) {
                ...
                for (int y = 0; y < 16; y++) {
                    for (int z = 0; z < 16; z++) {
                        for (int x = 0; x < 16; x++) {
                            int key = ((PaletteContainerAccessor) section.getBlockStateStorage()).getStorage().get(y << 8 | z << 4 | x);

                            if (key != 0) {
                                BlockState state = ((PaletteContainerAccessor) section.getBlockStateStorage()).getPalette().valueFor(key);

                                if (state != null) {
                                    int light = state.getLightLevel();

                                    if (light > 0) {
                                        pos.set(xBase + x, yBase + y, zBase + z);

                                        world.checkLight(LightType.BLOCK, pos);
                                    }
                                }
                            }
                        }
    ...
    public static void checkChunkLighting(final WorldChunk chunk, final World world) {
        ...
        for (int x = -1; x <= 1; ++x) {
            for (int z = -1; z <= 1; ++z) {
                if (x != 0 || z != 0) {
                    WorldChunk nChunk = world.getChunkSource().getLoadedChunk(chunk.chunkX + x, chunk.chunkZ + z);
